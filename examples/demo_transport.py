"""Synthesized STIM300 traffic for the functional-checkout demo.

Builds a ``FakeTransport`` pre-loaded with Init-Mode bytes, quiescent
Normal-Mode frames, Service-/Utility-Mode scripted replies, and a clean
Extended Error datagram, plus the matching ``ExpectedConfiguration``.

This module is the synthetic-device half of the demo; it is imported by
``examples/functional_checkout.py`` (and its pytest sibling) so the
checkout can run with no hardware attached. The real-hardware path uses
``SerialTransport`` instead and never touches this module.
"""
...
from pystim300 import ExpectedConfiguration, FakeTransport
...
from pystim300.normal import SPECIAL_SPECS, build_normal_frame
...
DEMO_PART_NUMBER = "8420005000000-A"
DEMO_SERIAL_NUMBER = "12345678901234"
...
def build_demo_transport(measurement_count: int) -> FakeTransport:
    ...
    isn_reply = _utility_reply("#isn,0,{0},".format(DEMO_SERIAL_NUMBER))
...
def demo_expected_configuration() -> ExpectedConfiguration:
    return ExpectedConfiguration(
        serial_numbers=frozenset({DEMO_SERIAL_NUMBER}),
        sample_rate_hz=2000,
        bit_rate=1843200,
